...
export default function App() {
  ...
  return (
    <>
    <style>{`@page { size: 80mm auto; margin: 1mm 0; } @media print { .app-root { display: none !important; } body { margin: 0; } }`}</style>
    ...
    {/* ЧЕК ДЛЯ ПЕЧАТИ — виден только при печати (Ctrl+P / кнопка "Распечатать чек"), формат под ленту 80мм, компактно */}
    {viewingBillOrder && (
      <div id="receipt-print-area" className="hidden print:block bg-white text-black" style={{ width: '72mm', margin: '0 auto', fontFamily: "'Courier New', monospace", lineHeight: 1.15 }}>
        
        <div style={{ textAlign: 'center', marginBottom: '2px' }}>
          <div style={{ fontSize: '14px', fontWeight: 900, letterSpacing: '1px' }}>HALIL ICE</div>
        </div>
        ...
        <div style={{ fontSize: '9px' }}>Официант: {viewingBillOrder.waiter}</div>
        ...
        <table style={{ width: '100%', fontSize: '10px', borderCollapse: 'collapse' }}>
          <tbody>
            {aggregateByDish(viewingBillOrder.items || []).map((item, idx) => (
              <tr key={idx}>
                <td style={{ fontWeight: 700, paddingTop: '1px' }}>{item.quantity}x {item.name}</td>
                <td style={{ textAlign: 'right', fontWeight: 700, whiteSpace: 'nowrap', paddingLeft: '4px' }}>{item.quantity * item.price}с</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ textAlign: 'center', fontSize: '9px', fontWeight: 700, marginTop: '3px' }}>
          Спасибо за заказ! 🙏
        </div>
      </div>
    )}
